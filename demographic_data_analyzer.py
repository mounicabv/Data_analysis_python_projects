...
def calculate_demographic_data(print_data=True):
    ...
    salary_above = df['salary'] == '>50K'

    # percentage with salary >50K
    higher_education_rich = round((higher_education & salary_above).sum() / higher_education.sum() * 100,1)

    lower_education_rich = round((lower_education & salary_above).sum() / lower_education.sum() * 100,1)
    ...
    rich_percentage = round((num_min_workers & salary_above).sum()/num_min_workers.sum()*100,1)
